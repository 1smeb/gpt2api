...
//
// 核心不变式:
//  1. 订单主链路为 pending -> paid -> completed; pending 也可转 expired/cancelled。
//  2. paid 的订单**只能由经过签名校验的异步回调**或管理员手工确认触发,履约必须幂等。
...
const (
	...
	StatusFulfillmentFailed = "fulfillment_failed"
	StatusFailed            = "failed" // 旧状态兼容:历史版本用于表示失败。
)
